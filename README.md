...
Это стандартный CSS-класс Tilda для карточек товаров в блоке T-Store. Атрибут `data-original`
содержит URL картинки до того как JavaScript заменит его через lazy-load. Скрипт берёт первый
такой элемент на странице — это и есть главное изображение для preload.

Если элемент не появился за 10 секунд — считается что товаров на странице нет, страница пропускается.

**Ограничение:** работает только со стандартным блоком T-Store в Tilda. Если используется
кастомный магазин или нестандартный блок — скрипт не найдёт изображения на таких страницах.
