...
**Шаг 6.** Проверьте любой сайт на наличие preload (настройка не нужна):

```
python -m tilda_vitals.cli check
```

Для автоматического исправления своего сайта — см. раздел «Автоматическое исправление» ниже:
...
## Быстрая проверка любого сайта

Команда `check` не требует настройки и работает с любым сайтом на Tilda — не только вашим.
На Linux/macOS запускается автоматически сразу после установки. На Windows запустите вручную:
