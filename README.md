...
По статистике рунета мобильный трафик делится на три семейства: Apple (~42%), Samsung/Xiaomi (~30%)
и прочий Android (~28%). Большинство iPhone используют ширину 390–414px при dpr=3, большинство
Android-флагманов — 393–414px при dpr=2.6–3. Для всех этих устройств Tilda CDN возвращает
одинаковый URL изображения — поэтому одного замера при 390px dpr=2 достаточно чтобы покрыть
~80% мобильного трафика.

Для десктопа топ-3 сегмента рунета — 1920px (37%), 1280px (20%), 1536px (8%) — все получают
одинаковый URL от CDN, поэтому замер при 1280px dpr=1 корректен для всего десктопного трафика.
